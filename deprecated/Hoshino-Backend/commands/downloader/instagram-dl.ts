...
export default {
    name: ['instagram-dl', 'igdl'],
    category: 'downloader',
    usage: ['instagram-dl <url>', 'igdl <url>'],
    async execute(args, { msg, socket }: ICTX) {
        const url = args[0]

        if (!url || !isValidUrl(url, 'instagram')) {
            socket.sendMessage(msg.remoteJid, { text: `❌ Please Provide Valid Instagram URL Please.` }, { quoted: msg.raw })
            return
        }
