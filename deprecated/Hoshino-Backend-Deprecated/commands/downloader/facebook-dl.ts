...
export default {
    name: ['facebook-dl', 'fbdl'],
    category: 'downloader',
    usage: ['facebook-dl <url>', 'fbdl <url>'],
    async execute(args, { msg, socket }: ICTX) {
        const url = args[0]

        if (!url || !isValidUrl(url, 'facebook')) {
            socket.sendMessage(msg.remoteJid, { text: `❌ Please Provide Valid Facebook URL Please.` }, { quoted: msg.raw })
            return
        }
